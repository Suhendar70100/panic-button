...
class UserController extends Controller
{
    public function index()
    {
        ...
        return view('user.index', compact('residentialData'));
    }
    public function dataTable(): JsonResponse
    {
        $data = User::with('residential')->where('username', '!=', 'lord_admin28')->orderBy('created_at', 'desc')->get();
        $data->map(function ($item) {
            $item->access = $item->access == 1 ? 'Bebaskan' : 'Batasi';
            $item->id_residential = $item->residential->name ?? '-';
            return $item;
        });
